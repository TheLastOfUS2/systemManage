		...
		$("#dg").datagrid({  
			...
			onLoadSuccess:function(data){
				...
				if(publicityType==0){
					...
					// $("#dg").datagrid("hideColumn", "publicityInterviewee");//受访人
					//更改列名
                    var option = $("#dg").datagrid("getColumnOption", "publicityTheme");
                    if(option.title!="材料名称"){
                        option.title = "材料名称";
                        $('#dg').datagrid();
                    }
				}else if(publicityType==1){
                    $("#dg").datagrid("hideColumn", "publicityCompany");//字数
                    $("#dg").datagrid("hideColumn", "publicityPosition");//层次
					//更改列名(判断当标题不为"入学时间"时更改列名)
					var option = $("#dg").datagrid("getColumnOption", "publicityTheme");
					if(option.title!="任务名称"){
						option.title = "任务名称";
						$('#dg').datagrid();
					}
					var option1 = $("#dg").datagrid("getColumnOption", "publicityTitle");
                    if(option1.title!="任务描述"){
                        option1.title = "任务描述";
                        $('#dg').datagrid();
                    }
                    var option2 = $("#dg").datagrid("getColumnOption", "publicityTime");
                    if(option2.title!="任务时间"){
                        option2.title = "任务时间";
                        $('#dg').datagrid();
                    }
				}else if(publicityType==2){
					//更改列名(判断当标题不为"会议名称"时更改列名)
					var option = $("#dg").datagrid("getColumnOption", "publicityTheme");
					if(option.title!="会议名称"){
						option.title = "会议名称";
						$('#dg').datagrid();
					}
					var option1 = $("#dg").datagrid("getColumnOption", "publicityTitle");
                    if(option1.title!="承担主要任务"){
                        option1.title = "承担主要任务";
                        $('#dg').datagrid();
                    }
                    var option2 = $("#dg").datagrid("getColumnOption", "publicityTime");
                    if(option2.title!="会议时间"){
                        option2.title = "会议时间";
                        $('#dg').datagrid();
                    }
					$("#dg").datagrid("hideColumn", "publicityVolume");//卷/期
	...
	function exportExcel(){
		...
		// 报纸
		if(publicityType==0){
			var url = "ReportServer?reportlet=publicityType0.cpt&publicityType=" + publicityType + "&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("材料撰写");
		// 期刊
		}else if(publicityType==1){
			var url = "ReportServer?reportlet=publicityType1.cpt&publicityType=" + publicityType + "&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("交办的其他任务");
		// 电视台
		}else if(publicityType==2){
			var url = "ReportServer?reportlet=publicityType2.cpt&publicityType=" + publicityType + "&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("会议服务");
		// 网络
		}else if(publicityType==3){
			var url = "ReportServer?reportlet=publicityType3.cpt&publicityType=" + publicityType + "&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("网络学院服务");
		}
		window.open(url);
	}
